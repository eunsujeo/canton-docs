...
## Musubi 만들 때 출발점
- `licensing/` 자리에 새 패키지(예: `settlement/`)를 만들어 **2-leg DvP 템플릿**을 작성.
- 패턴은 그대로: `SettlementRequest`(=LicenseRenewalRequest 역할) + `AllocationRequest` 인터페이스(transferLegs 2개) + `CompleteSettlement`(=CompleteRenewal, 원자적 실행).
