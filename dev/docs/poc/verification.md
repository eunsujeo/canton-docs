...
- **검증 가능성**: **제한적** —
  ...
  - *외부 필요*: **MM 익명**은 MM 측 뷰/스키마, **제3자 0건**은 거래에 무관한 노드 뷰가 있어야 입증 → 노드인프라가 제3자(비당사자) 노드 뷰 또는 RFQ 스키마를 제공해야 완전 검증([nodeinfra-asks.md](nodeinfra-asks.md) F).
- **합격 기준**:
...
> 노드인프라에 확인할 보안 질의는 [nodeinfra-asks.md](nodeinfra-asks.md) A·C.
