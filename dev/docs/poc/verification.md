...
- **감사추적/증빙** — 체결된 정산을 **Audit Exports**(체결 FXOrder + 4-leg `transactionHash`, CSV/JSON)·Statements로 **되짚어 검증하고 장부를 맞춰볼(대사)** 수 있는지 확인. (배치 엔드포인트는 예정 — 현재 per-intent 조회)
