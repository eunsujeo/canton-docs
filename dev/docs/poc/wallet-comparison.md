...
> 정산 흐름 자체는 [verification.md](verification.md) §6, 무스비 모델은 [musubi-overview.md](musubi-overview.md).
...
| 캔톤 파티 입도 | 거칠다(여러 주체가 한 파티로 수렴 가능) | 잘다(주체별 1급 파티) |
| 프라이버시·원자성 보장 | 파티 경계까지만(고객까지 안 내려감, DB 신뢰로 회귀) | 파티 단위로 원장이 강제 |
...
| 1차 PoC 적합성 | blind signing 검증 부담 → 1차엔 부담 | **적합 — 1차 기본** |
