...
- **SSE**(Server-Sent Events, 서버가 상태 변화를 실시간 push): 참여자별 SSE 엔드포인트, **서버 사이드 SSE 클라이언트**(국내은행 앱/백엔드; 브라우저 아님)로 연결(`intent_id` 필터, 30s heartbeat). Webhook 규약은 문서에 없음.
  - SSE는 국내은행이 **밖으로 연결(outbound)** → AWS Sandbox에 유리. Webhook은 무스비가 **국내은행 엔드포인트로 들어와야(inbound)** 해 망분리엔 번거로움 → 최종 단계에서 검토.
