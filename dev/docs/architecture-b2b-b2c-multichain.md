...
flowchart LR
  subgraph CAN2["Canton · 토큰표준 직접 발행 + B2B 정산"]
    KISS["KRW 발행자<br/>= KRWK 레지스트리"]
    JISS["JPY 발행자<br/>= JPYSC 레지스트리"]
    subgraph DVP2["Musubi DvP 정산"]
      direction TB
      ALK["국내은행: KRWK 잠금"]
      BLJ["해외은행: JPYSC 잠금"]
      EX2["venue 단일 트랜잭션<br/>KRWK↔JPYSC 동시 교환"]
      ALK --> EX2
      BLJ --> EX2
    end
  end
  KISS -->|"① KRWK mint"| ALK
  JISS -->|"① JPYSC mint"| BLJ
  EX2 -.->|"③ 상환: KRWK burn + 오프램프"| KISS
  EX2 -.->|"③ 상환: JPYSC burn + 오프램프"| JISS
```
1. **각 통화 발행자**가 자기 통화를 해당 기관에 **mint** — KRW 발행자→국내은행(KRWK), JPY 발행자→해외은행(JPYSC).
2. 두 기관이 잠금 → venue가 **단일 트랜잭션으로 KRWK↔JPYSC 동시 교환**(브릿지 불필요).
