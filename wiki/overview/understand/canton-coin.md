...
| 고려사항 | 접근 |
| --- | --- |
| **트래픽 예산 모니터링** | <abbr class="gloss" title="참여자 노드가 파티를 대신해 원장에서 활동(컨트랙트 저장·트랜잭션 제출·확인)해 주는 것. 로컬 파티는 키까지 노드가 관리하고, 외부 파티는 제출 키를 파티 자신이 보유(노드는 중계)">호스팅</abbr> 파티의 트래픽 크레딧 잔액 추적 |
...
⬅️ **이전**: [S11 — 정리·실습·심화](../../canton-course/s11-recap.md) ・ ➡️ **다음**: [Canton의 해법 — 세 가지 기둥](cantons-solution.md)
