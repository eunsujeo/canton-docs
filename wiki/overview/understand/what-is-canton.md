...
Canton은 모든 노드가 복제하는 단일 블록체인을 사용하지 않는다. 대신 **Synchronizer(synchronizer)**가 상태를 저장하지 않으면서 합의를 조율하고, **<abbr class="gloss" title="파티를 호스팅하고 그 파티의 컨트랙트를 저장·실행하는 노드. 밸리데이터의 핵심 구성요소">참여자 노드</abbr>(밸리데이터)**는 자신이 호스팅하는 파티와 관련된 데이터만 수신·저장한다.
