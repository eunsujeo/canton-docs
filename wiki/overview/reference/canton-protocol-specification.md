...
| **순서화 합의** | 일관된 <abbr class="gloss" title="상태를 저장하지 않고 트랜잭션 합의·순서를 조율하는 Canton 구성요소">동기화자</abbr> 순서 확립 | 시퀀서를 통한 BFT 순서화 | 동기화자 운영자 |
...
* **일관성(Consistency)**: 순서화 계층은 주어진 동기화자의 모든 상태 변경에 단일 전역 순서를 보장해 이중지불을 막는 데 기여한다.
