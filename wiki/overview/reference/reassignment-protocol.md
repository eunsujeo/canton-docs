...
언어사인이 제출되면 대상 Synchronizer에서 시간 증명이 요청된다. 이 타임스탬프 — **대상 타임스탬프** — 는 언어사인 처리 중 대상 Synchronizer 관련 검증(패키지 베팅, 이해관계자 호스팅 등)을 수행하는 데 쓰인다. 단일 고정 대상 타임스탬프를 쓰면 모든 <abbr class="gloss" title="이해관계자 밸리데이터가 트랜잭션이 유효함을 미디에이터에 응답하는 것(confirmation)">확인</abbr> 참여자가 같은 <abbr class="gloss" title="어떤 노드·파티·키가 네트워크에 참여하는지를 정의하는 구성 정보">토폴로지</abbr> 스냅숏에 대해 같은 검증을 실행하도록 보장한다.
