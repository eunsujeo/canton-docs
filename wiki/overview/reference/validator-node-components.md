...
**Wallet UI** — 밸리데이터 운영자(와 선택적으로 호스팅 사용자)가 Canton Coin 보유를 관리하고, <abbr class="gloss" title="원장 상태를 바꾸는 원자적 작업 단위. 하나 이상의 컨트랙트를 생성·보관하며, 전부 적용되거나 전혀 적용되지 않음">트랜잭션</abbr> 이력을 보고, 이전 오퍼를 수락·전송하고, 보상 수집을 모니터링하는 브라우저 기반 인터페이스. Wallet UI는 Validator App API와 통신한다.
