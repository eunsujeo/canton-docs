...
* **DSO 파티**: 확인 임계값 `t`로, 모든 SV 노드에 <abbr class="gloss" title="참여자 노드가 파티의 데이터·키를 맡아 두고, 그 파티를 대신해 원장에서 활동(저장·제출·확인)해 주는 것">호스팅</abbr>되는 DSO 파티라 부르는 탈중앙화 Daml 파티를 설정한다.
