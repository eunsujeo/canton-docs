...
- **핵심 용어**: 이해관계자 증명(Proof of Stakeholder), BFT 시퀀싱, 전체 순서(total order), <abbr class="gloss" title="Insanely Scalable State-machine replication. Canton 시퀀서가 메시지 전체 순서에 합의하는 데 쓰는 BFT 합의 알고리즘(1/3 미만 비잔틴 허용)">ISS</abbr> 알고리즘, 1/3 비잔틴 허용
...
3. 메시지들을 각 메시지의 권한 있는 수신자에게 **정해진 시퀀스 순서대로** 분배한다
4. 모든 수신자가 같은 순서를 보도록 보장한다

> 💡 **이 문장의 두 가지**: 
> ① **누구에게** — 각 메시지는 그 메시지의 *권한 있는 수신자에게만* 간다(라우팅). 
> ② **어떤 순서로** — 2번에서 매긴 *시퀀스 번호 순서*로 흘려보낸다(여러 메시지 사이의 순서). "순서"는 메시지 *내용*이 아니라 *메시지들 사이의 번호 순서*다.
>
> 예: M1(번호1, 수신권한 A·B), M2(번호2, B·C), M3(번호3, A·C) →
> **A**는 `M1 → M3`, **B**는 `M1 → M2`, **C**는 `M2 → M3`를 받는다. 각자 *자기 권한 메시지만* + *번호 순*으로 받고, 누구도 M3을 M1보다 먼저 보지 않는다(상대 순서 일치). 시퀀서는 내용을 모른 채 도착 순으로 번호만 매긴다.
...
* 노드의 1/3 미만이 비잔틴(악의·고장)이어도 견딘다 (그 이상이면 보장 깨짐)
...
> 💡 **"시퀀서의 순서화 계층을 통해 평결을 브로드캐스트"란?** 미디에이터가 밸리데이터에게 *직접* 연락하는 게 아니라, **평결(커밋/리젝트)을 하나의 메시지로 시퀀서에 실어** → 시퀀서가 순서를 매겨 **관련 밸리데이터 전원에게 전달**한다. 이유: **시퀀서가 Synchronizer의 유일한 통신 창구**라, 뷰·확인·평결 *모든* 메시지가 시퀀서를 통과해야 모두가 일관된 순서로 받는다. (미디에이터·밸리데이터는 서로 직접 연결돼 있지 않다.)
> 비유: 시퀀서 = **사내 방송 시스템.** 심판(미디에이터)이 각자에게 전화하는 대신, 결과를 방송실(시퀀서)에 넘기면 방송실이 관련자 전원에게 순서대로 송출한다.
