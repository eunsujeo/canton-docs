...
* **순서화 합의**는 전역 트랜잭션 순서를 확립한다. <abbr class="gloss" title="상태를 저장하지 않고 트랜잭션 합의·순서를 조율하는 Canton 구성요소">동기화자</abbr> 노드가 참여하지만, 암호화된 메시지만 본다.
...
순서화 계층은 한 동기화자의 모든 트랜잭션에 대한 **전체 순서(total order)** 를 확립한다. 이는 모든 참여자가 같은 동기화자로부터 같은 순서로 이벤트를 보게 해, 이중지불을 막고 일관성을 보장한다.
...
2. 그 동기화자에 대해 전역적으로 고유한 타임스탬프/시퀀스 번호를 부여한다
...
(<abbr class="gloss" title="슈퍼 밸리데이터들이 공동 운영하는 Canton의 퍼블릭 조율(합의) 계층">글로벌 동기화자</abbr> 같은) 탈중앙화 동기화자의 경우, 순서화는 비잔틴 장애 허용(BFT) 합의를 쓴다:
...
신뢰 가정: 시퀀서 노드의 1/3 미만이 악의적이다. 글로벌 동기화자에서는 이 신뢰가 독립적인 <abbr class="gloss" title="글로벌 동기화자를 운영하고 네트워크 거버넌스에 참여하는 노드">슈퍼 밸리데이터</abbr> 전반에 분산된다.
...
**유연한 신뢰 모델.** 서로 다른 동기화자가 서로 다른 순서화 신뢰 모델을 쓸 수 있다 — 프라이빗 배포에는 단일 운영자 시퀀서, 탈중앙화 네트워크에는 BFT 시퀀서 — 반면 스마트 컨트랙트 합의는 어느 경우든 동일하게 유지된다.
...
| **Canton** | 동기화자 (BFT) | 영향받는 이해관계자만 | 완전한 부분 트랜잭션 |
