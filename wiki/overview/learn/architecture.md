...
Canton은 **조율(coordination)** 과 **저장(storage)** 을 분리한다. Synchronizer는 트랜잭션 순서를 조율하고, 참여자 노드(밸리데이터)는 자신이 <abbr class="gloss" title="참여자 노드가 파티의 데이터·키를 맡아 두고, 그 파티를 대신해 원장에서 활동(저장·제출·확인)해 주는 것">호스팅</abbr>하는 파티의 데이터를 저장한다.
