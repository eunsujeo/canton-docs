...
밸리데이터는 Canton Network의 기본 인프라 단위다. <abbr class="gloss" title="Canton에서 권한과 데이터 가시성의 주체가 되는 식별 가능한 참여 주체">파티</abbr>를 호스팅하고, 그들의 컨트랙트 데이터를 저장하며, 그들을 대신해 <abbr class="gloss" title="원장 상태를 바꾸는 원자적 작업 단위. 하나 이상의 컨트랙트를 생성·보관하며, 전부 적용되거나 전혀 적용되지 않음">트랜잭션</abbr>을 처리한다. 밸리데이터 내부에 무엇이 있는지 이해하면 배포 계획, 문제 해결, 용량 산정에 도움이 된다.
