...
추가 트래픽 획득은 시퀀서 Admin API 접근과 시퀀서 간 동기화를 요구한다. Canton Network에서 이 동기화는 각 슈퍼 밸리데이터가 트래픽을 획득하는 멤버의 추가 트래픽 잔액을 갱신하는 Daml 워크플로를 통해 <abbr class="gloss" title="원장(Daml 컨트랙트) 위에서 실행·기록되는 것. 모든 이해관계자가 공유·검증·강제">온-원장</abbr>으로 처리된다.
