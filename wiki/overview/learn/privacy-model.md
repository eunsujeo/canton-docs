...
Canton은 **부분 트랜잭션 프라이버시(sub-transaction privacy)** 로 무결성-프라이버시 트레이드오프를 해소한다: 트랜잭션을 뷰로 분해해, 각 <abbr class="gloss" title="Canton에서 권한과 데이터 가시성의 주체가 되는 식별 가능한 참여 주체">파티</abbr>가 자기 부분을 검증하는 데 필요한 것만 보게 한다.
...
> ✅ **밸리데이터는** 자신이 <abbr class="gloss" title="참여자 노드가 파티를 대신해 원장에서 활동(컨트랙트 저장·트랜잭션 제출·확인)해 주는 것. 로컬 파티는 키까지 노드가 관리하고, 외부 파티는 제출 키를 파티 자신이 보유(노드는 중계)">호스팅</abbr>하는 파티의 데이터만 저장한다 — 전역 상태 복제 없음.
...
| **감사 초이스** | 감사 정보를 반환하는 <abbr class="gloss" title="실행해도 컨트랙트를 활성으로 남겨두는 초이스. 조회·알림·읽기에 쓴다">비소비형 초이스</abbr> 생성 |
...
> 💡 **구체 예시** (디벌전스 = "거래에 끌어다 쓴 컨트랙트가 그 거래 당사자에게 보이게 됨"):
> - **담보 통째 노출**: Alice가 Bob과 소액 거래를 하며 자기 **국채를 담보로 fetch**하면 → Bob은 "담보 충분"만 알면 되는데 그 **국채의 평가액·발행자·만기까지** 다 보게 됨.
> - **잔액 노출**: A가 B에게 100원 보낼 때 입력 보유(holding)를 fetch하면, 그 컨트랙트에 든 **A의 다른 잔액·메타데이터**까지 B가 볼 수 있음.
> - **완화**: 원본 자산을 끌어오지 말고, 필요한 사실만 담은 **작은 증명 컨트랙트**(예: "담보 충분함" 플래그)를 만들어 그것만 쓴다.
...
> 💡 **구체 예시** (내용은 암호화돼 못 봐도 "언제·얼마나·얼마나 크게"로 추론):
> - **M&A 낌새**: 평소 거래 없던 A↔B 기업 사이 트랜잭션이 갑자기 급증 → "큰 딜 진행 중"이라고 추측해 주가에 베팅.
> - **대형 주문 임박(선행매매)**: 트랜잭션 크기(메시지 용량)가 평소보다 큼 → "대형 거래 곧 정산"으로 읽고 시장에서 미리 포지션 잡음.
> - **주기 패턴**: 매달 25일 대량 트랜잭션 → "급여·배당 지급일"·사업 규모가 드러남.
> - 누가 보나? 메시지 흐름을 관찰하는 쪽(Synchronizer 운영자는 봉투의 타임스탬프·크기·수신처 메타데이터를 봄, 또는 활동을 지켜보는 거래 상대).
