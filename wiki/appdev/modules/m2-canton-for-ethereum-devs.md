...
| --- | --- | --- |
| 블록체인(Blockchain) | <abbr class="gloss" title="상태를 저장하지 않고 트랜잭션 합의·순서를 조율하는 Canton 구성요소">동기화자</abbr>(Synchronizer) | 상태를 저장하지 않고 합의 조율 |
...
* **서명자(Signatory)**: 컨트랙트 생성을 승인해야 하는 파티(들). 서명자는 항상 컨트랙트를 보며, 컨트롤러로도 선언되면 초이스를 실행할 수 있다.
* **관찰자(Observer)**: 컨트랙트를 볼 수 있는 파티이나, 컨트롤러로도 선언되지 않으면 초이스를 실행할 수 없다.
