...
Canton은 단지 문법이 다른 또 하나의 블록체인이 아니다. 분산 원장에 대한 근본적으로 다른 접근을 대표한다:
...
모든 정보를 조회할 수 있는 단일 "블록체인"은 없다. 각 <abbr class="gloss" title="Canton에서 권한과 데이터 가시성의 주체가 되는 식별 가능한 참여 주체">파티</abbr>는 원장에 대한 자신만의 뷰를 갖는다.
...
<abbr class="gloss" title="원장에 기록되는 불변 데이터 단위. 상태 변경은 새 컨트랙트 생성으로 표현됨">컨트랙트</abbr>는 변하지 않는다. 컨트랙트를 "업데이트"할 때는 옛것을 보관하고 새것을 생성한다. 이는 한계가 아니라 프라이버시·무결성 보장의 토대다.

### 명시적 권한

각 파티가 무엇을 할 수 있는지 컴파일 타임에 선언하고, 프로토콜이 그것을 강제한다. 런타임에 호출자 신원을 확인하는 전통적 시스템과 달리, Canton의 권한은 구조적(structural)이다.
