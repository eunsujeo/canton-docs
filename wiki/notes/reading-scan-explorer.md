...
라운드 N:  열림(open) → 발행중(issuing) → 닫힘(closed)
            ↑활동 받음    ↑보상 지급(여기서 받음)  ↑기간 종료·자동 정리
```
> 비유: 라운드 = **급여 기간**. 열림=일하는 중, 발행중=**월급 지급**, 닫힘=기간 마감(장부 <abbr class="gloss" title="컨트랙트를 소비해 비활성으로 만드는 것(archive). 보관된 컨트랙트는 더 이상 쓸 수 없음">보관</abbr>).
> ⚠️ **용어 주의**: 라운드가 끝나는 건 **결산(기간 마감)** 이다. Musubi의 **정산(<abbr class="gloss" title="인도-대-지급(Delivery vs Payment). 자산 인도와 대금 지급을 동시·원자적으로 처리">DvP</abbr> settlement, 거래 마무리)** 과 헷갈리지 말 것 — 다른 개념.
> ⚠️ **"닫힌 라운드 = 0"의 함정**: `closed-rounds` API는 *지금 <abbr class="gloss" title="거래·컨트랙트가 기록되는 장부. Canton에선 활성 컨트랙트의 모음">원장</abbr>에 남은 닫힘-대기 <abbr class="gloss" title="원장에 기록되는 불변 데이터 단위. 상태 변경은 새 컨트랙트 생성으로 표현됨">컨트랙트</abbr>만* 센다. 닫힌 라운드는 **금방 자동 정리(archive)** 돼서 보통 0으로 보인다. **"0 = 안 닫힘"이 아니다.** 실제 닫힘 여부는 open/issuing 목록에서 *사라졌는지*로 판단(예: open=[19,20,21]·issuing=[16,17,18]이면 1~15는 이미 닫힘).
...
- 라운드는 열림→발행중→닫힘으로 흐르고, **닫힌 라운드는 자동 정리**되어 `closed-rounds` 목록엔 거의 안 남는다(위 ⚠️ 참고).
...
- 실측 예(LocalNet): round 1이 02:03 열림 → **02:23 닫힘 예정**. 한 라운드가 닫히기까지 **20분 남짓** 걸린다는 뜻.
- 결산은 **SV 자동화가 자동으로** 수행한다(사람 할 일 없음). 닫힌 라운드는 **곧바로 정리(archive)** 되므로 `closed-rounds`/`닫힌 라운드`는 보통 0으로 보인다 — 진행 여부는 **open/issuing 라운드 번호가 오르는지**로 본다.
...
#### 실측 사례 — Tap한 CC는 즉시 트래픽으로 소비됨 (CC ↔ 트래픽)
LocalNet에서 app-user 잔액을 추적하다 발견한 것(헷갈리기 쉬운 핵심):
- round 1에 **<abbr class="gloss" title="DevNet·LocalNet에서 테스트용 CC를 무료로 받는 동작(수도꼭지/faucet). MainNet엔 없음">Tap</abbr> +4,000.8 CC** 를 받자마자, **같은 순간 `Sent (Extra Traffic Purchase)` −4,000.8 CC** 로 전액 소비됐다.
- 즉 온보딩 자동화가 "거래할 수 있게" **무료 CC를 받자마자 트래픽(=<abbr class="gloss" title="상태를 저장하지 않고 트랜잭션 합의·순서를 조율하는 Canton 구성요소">Synchronizer</abbr>에 쓰기 위한 자원)으로 전환**한 것. (CC를 내고 트래픽을 산다 → CC 소각)
- 그래서 지갑 **Total Available Balance**에는 Tap CC가 **안 남고**, **보상 코인만** 표시된다(매 라운드 +570씩 누적). 부제 "*unlocked* Canton Coin and rewards earned"가 이를 가리킴.
- 트래픽 구매는 **Scan 공개 활동 피드엔 안 잡히고**(scan은 CC 경제 활동만), **지갑의 Transactions 내역에만** 보인다.

> 교훈: "받은 CC가 잔액에 왜 없지?"의 답은 보통 **트래픽으로 썼다**. CC↔트래픽 관계의 실제 작동. (지갑 잔액 = 보통 *available=unlocked* 코인만)

### 3. 총 CC(Amulet) 잔액
- <abbr class="gloss" title="다자간 워크플로를 위해 설계된 Canton의 스마트 컨트랙트 언어">Daml</abbr>/Scan에서 **Canton Coin의 기술적 이름이 Amulet**이다 (CC = Amulet).
- "Could not retrieve total amulet balance"는 **에러가 아니라**, 닫힌 라운드 스냅샷에서 계산하는 **네트워크 전체 합계 통계가 아직 없을 때**의 메시지다. (실측: 이 LocalNet에선 라운드가 21까지 가도 이 통계가 안 떴다 — 별도 집계라 환경에 따라 비어있을 수 있음)
- **개별 파티 잔액**이 궁금하면 이 통계 말고 **각자 Wallet의 "Total Available Balance"** 를 보면 된다(거기엔 정상 표시).
...
| 화면 표시 | API 타입 | 뜻 |
|---|---|---|
| **Tap** | `devnet_tap` | **무료 CC 받기(수도꼭지/faucet)**. DevNet·LocalNet에서만 가능 — 테스트용 CC를 "탭"해 받는다. **MainNet엔 없음**(거기선 실제로 벌어야 함). |
