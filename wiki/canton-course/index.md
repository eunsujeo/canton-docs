...
블록체인 기본은 알지만 **Canton은 처음**인 백엔드 개발자를 위한 12단계 코스. 매 단계는 **질문 하나**로 열고, **기초**(친숙한 것과의 비교표·이더리움 비교·전통[국경 간] 비교)에서 **심화**(실제 <abbr class="gloss" title="Canton에서 권한과 데이터 가시성의 주체가 되는 식별 가능한 참여 주체">파티</abbr> ID·<abbr class="gloss" title="다자간 워크플로를 위해 설계된 Canton의 스마트 컨트랙트 언어">Daml</abbr> 스니펫·choice 시퀀스 같은 구체 값)로 내려간다.

## 이 코스가 답하는 질문
- "은행 송금·정산을 블록체인으로 한다는데, 이더리움하고 뭐가 다른가?"
- "데이터는 어디 저장되나? 내 거래를 제3자가 보나? 키는? 주소는? 코드는 어떻게 생겼나?"
- "되돌릴 수 없나? 누가 순서를 정하나? 실제로 무엇을 띄워야 도나?"

## 대상
- HTTP API·DB·인증(JWT) 같은 백엔드 기본기가 있는 개발자.
- 블록체인 일반(<abbr class="gloss" title="원장 상태를 바꾸는 원자적 작업 단위. 하나 이상의 컨트랙트를 생성·보관하며, 전부 적용되거나 전혀 적용되지 않음">트랜잭션</abbr>·키·<abbr class="gloss" title="여러 노드가 트랜잭션의 유효성·순서에 함께 동의하는 절차">합의</abbr>·<abbr class="gloss" title="원장 위에서 규칙대로 자동 실행되는 코드화된 계약. Canton에선 Daml 템플릿으로 작성">스마트 컨트랙트</abbr>)은 들어봤고, 이더리움을 비교 기준으로 삼을 수 있는 사람.
- Canton·Daml 경험은 없어도 된다.

## 읽는 법
- **순서대로** 읽는다. 각 단계는 앞 단계를 전제로 한다.
- 어려운 용어는 본문 첫 등장에 마우스오버 정의(점선 밑줄)가 붙는다. 더 궁금하면 [용어집](../glossary.md).
- 각 단계 하단의 **강의 노트**는 이 내용을 남에게 설명할 때 쓰는 메모다(핵심 한 문장·비유·예상 질문).

## Canton 한눈에
Canton은 **공개-허가형(public-permissioned) 프라이버시 보존 네트워크**다. 하나의 거대한 글로벌 체인이 아니라:

- **<abbr class="gloss" title="파티를 호스팅하고 그 파티의 컨트랙트를 저장·실행하는 노드. 밸리데이터의 핵심 구성요소">참여자 노드</abbr>**(<abbr class="gloss" title="파티를 호스팅하고 그 파티의 컨트랙트 데이터를 저장하는 참여자 노드">밸리데이터</abbr>)들이 각자 자기 파티의 데이터를 <abbr class="gloss" title="컨트랙트를 소비해 비활성으로 만드는 것(archive). 보관된 컨트랙트는 더 이상 쓸 수 없음">보관</abbr>하고,
- **<abbr class="gloss" title="상태를 저장하지 않고 트랜잭션 합의·순서를 조율하는 Canton 구성요소">Synchronizer</abbr>**가 트랜잭션의 순서·확정만 조율한다(내용은 안 봄).
...
| [S3](s03-daml-contract.md) | Daml 컨트랙트 | 규칙은 어떻게 코드가 되나? 솔리디티와 뭐가 다른가? |
